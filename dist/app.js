...
const bannerInitialVideo = document.querySelector("#banner-initial-video");
const bannerInitialContent = document.querySelector("#banner-initial-content");

let hasBannerMainVideoPlayed = false;
...
const bannerTimeline = gsap.timeline({
  scrollTrigger: {
    ...
    pinSpacing: true,
    onEnter: () => {
      if (!hasBannerMainVideoPlayed) {
        playVideo(bannerInitialVideo);
        gsap.to(bannerInitialContent, {
          opacity: 0,
          scale: 0.95,
          zIndex: 0,
          pointerEvents: "none",
...
bannerTimeline
  ...
  .to(
    "#after-scroll-banner h3",
    {
      y: -(window.innerHeight + 200),
      ease: "none",
      stagger: 0.4,
    },
    "afterScrollBannerAnim"
  )
  .to(
    "#after-scroll-videos video",
    {
      opacity: 1,
      zIndex: 1,
      duration: 0.01,
...
const headerTimeline = gsap.timeline({
  scrollTrigger: {
    ...
    onLeave: () => {
      document
        .querySelector("header nav a:nth-child(4)")
        .classList.remove("hover:bg-white", "hover:text-black");
      document
        .querySelector("header nav a:nth-child(4)")
        .classList.add("hover:bg-black", "hover:text-white");
    },
    onLeaveBack: () => {
      document
        .querySelector("header nav a:nth-child(4)")
        .classList.remove("hover:bg-black", "hover:text-white");

      document
        .querySelector("header nav a:nth-child(4)")
        .classList.add("hover:bg-white", "hover:text-black");
    },
  },
});
...
// let arr = [];
// for (let i = 0; i <= 197; i++) {
//   arr.push(`./assets/images/canvas2/${i < 10 ? "0" + i : i}.jpeg`);
// }
// console.log(arr);
